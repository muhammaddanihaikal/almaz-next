// removed top-level import to speed up initial page load
...
/**
 * Returns a Date object representing the current time in Jakarta (UTC+7)
 */
...
/**
 * Returns current date as YYYY-MM-DD string in Jakarta time
 */
...
export const getDateRanges = () => {
  ...
  const fmt = (d) => {
    // We use the local parts of the Date object which now represents Jakarta time
...
const buildSheet = (rows, columns, meta = [], options = {}) => {
  ...
  // Compute consecutive merges
  const merges = []
  if (options.mergeConsecutive && Array.from(options.mergeConsecutive).length > 0) {
    const colIndices = options.mergeConsecutive
    const dataStartRowIdx = headerRowIdx + 1

    colIndices.forEach((colIdx) => {
      let startRow = dataStartRowIdx
      for (let R = dataStartRowIdx; R < sheetRows.length; R++) {
        const currentVal = sheetRows[R][colIdx]
        const nextVal = R + 1 < sheetRows.length ? sheetRows[R + 1][colIdx] : null
    ...
    for (let R = range.s.r; R <= range.e.r; R++) {
      for (let C = range.s.c; C <= range.e.c; C++) {
        ...
        if (R >= headerRowIdx) {
          ...
          const shouldCenter = options.centered || 
                              (options.centerData && R >= headerRowIdx) || 
                              (options.centerColumns && options.centerColumns.includes(C))
