...
export function buildRincianPerSalesData(rows, rokokList) {
  ...
  const processedKonsinyasiIds = new Set()
  const processedTukarIds = new Set()
  ...
  function addData(rokokId, salesName, delta) {
    ...
    if (langsungQty === 0 && titipQty === 0 && tukarQty === 0 && langsungUang === 0 && titipUang === 0 && tukarUang === 0) return
  ...
  for (const sesi of rows) {
    ...
    // 3. Tukar Barang Selesai — gunakan logika per produk (seperti Rekap Harian)
    // Keluar = penjualan (+qty, +uang), Masuk = retur (-qty, -uang)
    ...
    // 4. Retur Barang (tanpa pengganti) — mengurangi penjualan langsung
    ...
    for (const retur of sesi.returDiSesi || []) {
      for (const it of retur.items || []) {
        if (!(it.qty > 0)) continue
        const hargaRetur = hargaAktualSesi[it.rokok_id] || rokokMeta[it.rokok_id]?.harga_toko || 0
        addData(it.rokok_id, salesName, { langsungQty: -it.qty, langsungUang: -it.qty * hargaRetur })
      }
    }
  }
  ...
  if (activeSales.length > 0) {
    const rokokIdSet = new Set()
    // Tampilkan semua rokok aktif
    rokokList.filter((r) => r.aktif !== false).forEach((r) => rokokIdSet.add(r.id))
    // Dan tambahkan juga rokok yang punya transaksi (historical/inactive)
    allRokokIds.forEach((id) => rokokIdSet.add(id))
    
    sortedRokokIds = [...rokokIdSet].sort((a, b) => (rokokIdOrderMap[a] ?? 0) - (rokokIdOrderMap[b] ?? 0))
  }
