...
export async function mutateStock({
  ...
  const doWork = async (db) => {
    ...
    const lockedRows = await db.$queryRaw`
      SELECT "stok", "stok_sample_cukai", "stok_sample_biasa" FROM "public"."Rokok" WHERE id = ${rokok_id} FOR UPDATE
    `;
...
export async function mutateStockBatch({ tx, mutations }) {
  ...
  const lockedRows = await tx.$queryRaw`
    SELECT id, stok, stok_sample_cukai, stok_sample_biasa FROM "public"."Rokok" WHERE id IN (${Prisma.join(rokokIds)}) FOR UPDATE
  `;
...
export async function rebuildAllStockCache() {
  await prisma.$transaction(async (tx) => {
    ...
    for (const r of allRokok) {
      await tx.$executeRaw`SELECT 1 FROM "public"."Rokok" WHERE id = ${r.id} FOR UPDATE`;
