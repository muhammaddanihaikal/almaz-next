  ...
  await prisma.$transaction(async (tx) => {
    const r = await tx.rokok.create({
      data: {
        ...
        harga_retail: Number(data.harga_retail),
    ...
    await logAudit({
      ...
      new_values:  { nama: r.nama, stok: r.stok, harga_beli: r.harga_beli, harga_grosir: r.harga_grosir, harga_retail: r.harga_retail, harga_perorangan: r.harga_perorangan },
      user_id:     userId,
...
export async function updateRokok(id, data, alasan) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await logAudit({
      ...
      old_values:  { nama: old.nama, stok: old.stok, harga_beli: old.harga_beli, harga_grosir: old.harga_grosir, harga_retail: old.harga_retail, harga_perorangan: old.harga_perorangan },
      new_values:  { nama: data.nama, stok: newStok ?? old.stok, harga_beli: Number(data.harga_beli), harga_grosir: Number(data.harga_grosir), harga_retail: Number(data.harga_retail), harga_perorangan: Number(data.harga_perorangan) },
      alasan,
      user_id:     userId,
      user_name:   session?.user?.name,
    })
...
export async function deleteRokok(id, alasan) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await logAudit({
      ...
      old_values:  { nama: old.nama, harga_beli: old.harga_beli, harga_grosir: old.harga_grosir, harga_retail: old.harga_retail, harga_perorangan: old.harga_perorangan },
      alasan,
      user_id:     userId,
      user_name:   session?.user?.name,
    })
